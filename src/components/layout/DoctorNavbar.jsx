...
const doctorNotifications = [
  { id: 1, title: "New appointment request", detail: "Ayesha Malik requested a follow-up visit", time: "20 min ago", unread: true },
  { id: 2, title: "Lab results uploaded", detail: "Ali Hassan's blood work is ready for review", time: "1 hr ago", unread: true },
  { id: 3, title: "Schedule reminder", detail: "You have 3 appointments today", time: "3 hrs ago", unread: false },
];

export default function DoctorNavbar({ setMobileOpen, pageTitle }) {
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden sm:block text-left leading-tight">
                <p className="text-sm font-medium text-ink-800">{currentDoctor.name}</p>
                <p className="text-[11px] text-ink-400">{currentDoctor.specialization}</p>
              </div>
            ...
            <AnimatePresence>
              {profileOpen && (
                ...
                >
                  <button onClick={() => { navigate("/doctor/profile"); setProfileOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-ink-600 hover:bg-ink-50">
                    <UserCircle className="w-4 h-4" /> My Profile
                  </button>
                  <button onClick={() => { navigate("/doctor/settings"); setProfileOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-ink-600 hover:bg-ink-50">
